...
const Metrics = (() => {
      ...
      <div class="card mt-3">
        ...
        <div class="table-wrap" style="border:none;">
          <table class="table">
            ...
            <tbody>
              ${metrics.length === 0 ? '<tr><td colspan="10" style="text-align:center;padding:30px;color:var(--text-muted);">尚無數據，點擊右上「填寫今日」開始</td></tr>' : ''}
              ${metrics.map(m => {
  ...
  const openForm = (metricId = null) => {
    ...
    const metric = metricId
      ...
      : data.dailyMetrics.find(m => m.date === today);
    ...
    const html = `
      <form id="metricForm" class="form-grid">
        ...
        <div class="form-field">
          <label>責任人</label>
          <input name="owner" value="${escapeHtml(metric?.owner || '林業務')}">
        </div>
  ...
  const save = (metricId) => {
    ...
    if (metricId) {
      ...
      // 同日期已存在 → 覆蓋
      const existIdx = data.dailyMetrics.findIndex(m => m.date === obj.date);
      if (existIdx >= 0) {
